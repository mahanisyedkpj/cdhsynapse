...
def render_search(df):
    ...
    with c_list:
        if not filtered.empty:
            ...
            with header_cols[3]: st.markdown("<div class='specialist-row-header'>Ongoing Research</div>", unsafe_allow_html=True)

            def research_summary(value, label):
                if pd.isna(value) or str(value).strip() == "":
                    return f"No {label.lower()} research"
                items = [
                    item.strip()
            ...
            for _, row in filtered.iterrows():
                ...
                published_summary = research_summary(row.get("PublishedResearch", ""), "Published")
                ongoing_summary = research_summary(row.get("OngoingResearch", ""), "Ongoing")
                ...
                with row_cols[2]:
                    st.markdown(f"<div class='specialist-row-cell {'selected' if is_selected else ''}' style='color:#666;'>{published_summary}</div>", unsafe_allow_html=True)
                with row_cols[3]:
                    st.markdown(f"<div class='specialist-row-cell {'selected' if is_selected else ''}' style='color:#666;'>{ongoing_summary}</div>", unsafe_allow_html=True)
